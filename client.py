...
class ColMCRClient(object):
    def __init__(self, client_id, data_set_train, data_set_test, num_classes, neig, num_neig, device, path):
        """Client object is initiated by the center server."""
        self.id = client_id
        self.dataset_train = data_set_train
        self.dataset_test = data_set_test
        ...
        self.neig = neig
        self.num_neig = num_neig
        self.neig_z = torch.tensor(0)
        self.neig_z_label = torch.tensor(0)
        self.__netD = None
        self.__netC = None
        self.path = path
    ...
    def client_pretrain(self):
        """Update local model using local dataset."""
        loss, loss_term1, loss_term2 = self.pretrainer.train(\
            self.neig_z.to(self.device), self.neig_z_label.to(self.device))
        self.netD.to("cpu")
        if self.device == "cuda": torch.cuda.empty_cache()
    ...
    def client_pretest(self):
        ...
        with torch.no_grad():
            ...
            for idx2, (data_test, label_test) in enumerate(dataloader_test):
                ...
                colmcr_loss = ColMCRLoss(gamma1 = 1.0, gamma2 = 1.0, eps=0.5, numclasses=self.num_classes, num_neig=self.num_neig)
                err, _, _ = colmcr_loss(feature_test, label_test, self.neig_z.to(self.device), \
                                  self.neig_z_label.to(self.device))
    ...
    def getz(self):
        ...
        self.netC.eval()
        with torch.no_grad():
            dataloader = torch.utils.data.DataLoader(self.dataset_train, batch_size = 64, shuffle=True)
            for idx, (data, label) in enumerate(dataloader):    
                real_cpu = data.to(self.device)
                Z = self.netD.to(self.device)(real_cpu)
                break
